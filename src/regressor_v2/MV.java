...
public final class MV {
	...
	static final boolean  BOOLEAN_OUTPUT = false;
	static final int MAX_NEW_FUNC_LEN = 10000, MAX_NEW_FUNC_DEPTH = 6, TSIZE = 2,
	...
	static int numOfInitialInputs, numOfCurrInputs, numOfTestCases_all, numOfTestCases_curr, numTruePositives_curr,
	...
	public static ArrayList<DataEntry> createDataEntries (int[] inputIndexes, int outputIndex){
		ArrayList<DataEntry> newDataEntries = new ArrayList<DataEntry>();
		double output;
		ArrayList<Double> inputs = new ArrayList<Double>();
		
		for (int i = 0; i < allData.size(); i++) {
			for (int j=0; j<inputIndexes.length; j++){
				inputs.add(allData.get(i).getInputAtIndex(j));
			}
			output = allData.get(i).getInputAtIndex(outputIndex);
			DataEntry DE = new DataEntry(inputs, output);
			newDataEntries.add(DE);
		}
	...
	public static void createPredictionBasedDataSet(int maxSize) {
		int size = Math.min(maxSize, allData.size());
		ArrayList<DataEntry> tempData = new ArrayList<DataEntry>();
		...
		for (int i = 0; i < size; i++) {
			int index1 = rd.nextInt(tempData.size());
			int index2 = rd.nextInt(tempData.size());
			
			if (tempData.get(index1).correctPredictiosNumber < tempData.get(index2).correctPredictiosNumber) {
				resultData.add(tempData.get(index1));
				tempData.remove(index1);
			} else {
				resultData.add(tempData.get(index2));
				tempData.remove(index2);
			}
		}
